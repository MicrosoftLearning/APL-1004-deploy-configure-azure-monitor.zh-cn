...
1. 在 Azure 门户搜索栏中，输入**资源组**并从结果列表中选择**资源组**。
1. 在**资源组**页上，选择**创建**。
1. 在 **创建资源组**页上，选择订阅并输入名称 rg-alpha。 将区域设置为美国东部，选择**查看 + 创建**，然后选择**创建**。

> [!NOTE]
> 这组练习假定您选择部署在美国东部地区，但如果您选择，也可以将其更改为其他地区。 请记住，每次看到这些说明中提到美国东部地区时，都需要用您选择的地区来代替。

## 创建应用程序日志检查员安全组

在本练习中，您将创建一个 Entra ID 安全组。

1. 在 Azure 门户搜索栏中，从结果列表中输入 Azure Active Directory（或 Entra ID）和 Azure Active Directory（或 Entra ID）。
1. 在**默认目录**页面上，选择**组**。
1. 在**组**页面上，选择**新建组**。
1. 在**新建组**页面，提供下表中的值，然后选择**创建**。
...
在本练习中，您将部署和配置 Windows Server 虚拟机。

1. 在 Azure 门户搜索栏中，输入**虚拟机**并从结果列表中选择**虚拟机**。
1. 在**虚拟机**页面依次选择**创建**、**Azure 虚拟机**。
1. 在创建一台虚拟机向导的**基本信息**页面，挑选以下设定并选择**查看 + 创建**。
...
在本练习中，你将部署和配置一个 Linux 虚拟机。

1. 在 Azure 门户搜索栏中，输入**虚拟机**并从结果列表中选择**虚拟机**。
1. 在**虚拟机**页面依次选择**创建**、**Azure 虚拟机**。
1. 在创建一台虚拟机向导的**基础**页面，挑选以下设定并选择**查看 + 创建**。
...
4. 查看信息并选择**创建**。
1. 部署虚拟机后，打开**虚拟机属性**页面，选择**设置**下的**扩展 + 应用**程序。
1. 选择**添加**并选择**Network Watcher Agent for Linux**。 选择**下一步**，然后选择**审查和创建**。 选择**创建**。
